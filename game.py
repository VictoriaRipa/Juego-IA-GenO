...
class Game:
    ...
    def update(self, delta_time):
        ...
        if self.lost or not self.started:
            for event in events:
                #Cuando perdimos o no empezamos, ENTER empieza el juego
                if event.type == KEYDOWN and event.key == K_RETURN:
                    self.initialize()
                    self.started = True
        else:
            #Aumentar la velocidad segun el tiempo que duramos
            ...
            for event in events:
                if event.type == ADD_ENEMY:
                    ...
                    #Actualizar el timer que define cuando aprece un nuevo enemigo
